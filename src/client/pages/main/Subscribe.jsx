...
class Subscribe extends React.Component{
    ...
    async emailSubscription() {
        ...
        await fetch(`https://one-aviation.herokuapp.com/api/v1/subscription/subscribe?email=${this.state.email}`, {
            method: 'PUT',
    ...
    validateSubscription = () => {
        ...
        if (this.state.email_sub === "") {
            ...
            if (!this.state.email_sub.includes('@') || !this.state.email_sub.includes('.')){
    ...
    render(){
                    ...
                    <Row>
                        <Col md={5}>
                            <Card id="card-1" data-aos="fade-right" data-aos-offset="200" data-aos-easing="ease-in-sine">
                                ...
                                <input className="enter-input" id="name" value={this.state.name} onChange={e => this.setState({name: e.target.value})}></input>
                                ...
                                <textarea className="enter-input" id="message" value={this.state.message} onChange={e => this.setState({message: e.target.value})}></textarea>
                                <div className="form-errors">{this.state.messageError}</div>
                                <div className="btn-subscribe">
                                    <button onClick={(e) => this.handleSubmitMessage(e)}>Submit</button>
                                </div>
                                </form>
                            </Card>
                            {this.state.message_success === true ?
                            <Alert variant={'success'} onChange={this.alertDisable()} className="subscribe-alert">
     ...
     data-aos-anchor-placement="top-bottom" data-aos-easing="ease-in-sine" data-aos-offset="200" data-aos-delay="100">Subscribe & Send Us A Message</h1>
                            <Card id="card-2" data-aos="fade-left" data-aos-easing="ease-in-sine" data-aos-delay="200">
                                ...
                                <label for="email">Email</label>
                                <input className="enter-input" id="email" value={this.state.email_sub} onChange={e => this.setState({email_sub: e.target.value})}></input>
                                <div className="form-errors">{this.state.emailError}</div>
                                <div className="btn-subscribe">
                                    <button onClick={(e)=> this.handleSubmitSubscription(e)}>Subscribe</button>
                                </div>
                                </form>
                            </Card>
