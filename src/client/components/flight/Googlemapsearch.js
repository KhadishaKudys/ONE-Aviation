...
function Map(){
    ...
    function selectPort(port){
        ...
        if(location === 'from'){
            localStorage.setItem('from_latitude', port.latitude);
            localStorage.setItem('from_longitude', port.longitude);
            localStorage.setItem('departure_port', port.name);
        } else if(location === 'to') {
            localStorage.setItem('to_latitude', port.latitude);
            localStorage.setItem('to_longitude', port.longitude);
    ...
    return (
        <GoogleMap defaultZoom={6} defaultCenter={{lat:43, lng:13}}>
            ...
            {selectedLocation && (
                <InfoWindow position={{lat: selectedLocation.latitude, lng: selectedLocation.longitude}}
                    onCloseClick={() => {
                        setSelectedLocation(null)
                    }}>
                    <div>
                        <h2>{selectedLocation.name}</h2>
                        <button onClick={() => selectPort({latitude: selectedLocation.latitude, longitude: selectedLocation.longitude, name: selectedLocation.name})}>Select</button>
                    </div>
                </InfoWindow>
            )}
        </GoogleMap>
